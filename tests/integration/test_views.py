...
def test_should_call_callback_command_function(mocker: MockerFixture, client_request: Client) -> None:
    ...
    response = client_request.post(endpoint_url, data=request_body, content_type=json_content_type)
...
def test_should_return_error_when_callback_function_path_body_attribute_is_not_provided(
    ...
    body = {}  # empty body
    response = client_request.post(endpoint_url, data=body, content_type=json_content_type)
